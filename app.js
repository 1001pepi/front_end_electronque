    ...
    for(let i = 0; i<users.length; i++){
      emailScheduler[users[i].email] = setInterval(() => {
        ...
        var mailOptions = {
          ...
          text: 'Bonjour, votre consommation d\'energie est de: ' + value.finalEnergyValue + ' Wh qui correspond a un cout de: ' + value.finalEnergyValue * 0.05 + " FCFA"
        };
...
//récupérer la liste des consommations
app.get('/api/historique', getHistorique)

async function getHistorique(req, res){
  const historique = await Energy.find();
